...
class DataPreprocessing:
    """Creates the DataPreprocessing class

    Construction:: dp = DataPreprocessing(raw_path="sample Raw df path", target_path="sample Target df path",
    df_raw_types="sample Raw data types", df_target_types="sample Target data types")
    ...
    def __init__(self, raw_path=None, target_path=None,
        ...
        # Initialisation of DataPreprocessing class.
    ...
    def __clean_column_names(self):
        ...
        for name, df_dict in self.dfs.items():
            for col in df_dict['df'].columns:
                df_dict['df'].rename(columns={col: col.lower().strip().replace(":", "")
        ...
        for name, df_dict in self.dfs.items():
            if df_dict['df_dtypes']:
                ...
                for i in df_dict['df_dtypes']['datetime_cols']:
                    df_dict['df'][i] = pd.to_datetime(df_dict['df'][i], errors='coerce')
                # Integer variables
                for i in df_dict['df_dtypes']['int_cols']:
                    df_dict['df'][i] = pd.to_numeric(df_dict['df'][i], errors='coerce', downcast='integer')
                # Float variables
                for i in df_dict['df_dtypes']['float_cols']:
                    df_dict['df'][i] = pd.to_numeric(df_dict['df'][i], errors='coerce')
    ...
    def __drop_duplicate_obs(self):
        ...
        for name, df_dict in self.dfs.items():
            ...
            logging.info(f'Dropped {len_init - df_dict["df"].shape[0]} rows from the {name} dataset.')
    ...
    def __initial_eda(self):
        ...
        if self.generate_plots:
            ...
            for name, df_dict in self.dfs.items():
                ...
                len_cat = len(df_dict['df'].select_dtypes(include=['category', 'object']).columns)
                if len_cat > 0:
                    fig, ax = plt.subplots(4, math.ceil(len_cat / 4), figsize=(len_cat * 2, len_cat * 3))
                    for variable, subplot in zip(df_dict['df_dtypes']['categ_cols'], ax.flatten()):
                        sns.countplot(df_dict['df'][variable], ax=subplot).set_title(f'Histogram of {variable}')
                ...
                numer_cols = df_dict['df'].select_dtypes(exclude=['category', 'object', 'datetime']).columns
                if len(numer_cols) > 0:
                    df_dict['df'][numer_cols].hist(bins=15, figsize=(len(numer_cols) * 2, len(numer_cols) * 3),
    ...
    def __find_index_column(self):
        ...
        if 'index' in self.dfs['Raw']['df'].columns:
            ...
            similarity_dict = {}
            # To find potential index column in Raw dataset, compare all Raw columns with Index column in Target dataset
            for un_col in self.dfs['Raw']['df'].columns:
                res = len(
                    set(self.dfs['Raw']['df'][un_col].unique()) & set(self.dfs['Target']['df']["index"].unique())) / \
                      float(len(set(self.dfs['Raw']['df'][un_col].unique()) | set(
                          self.dfs['Target']['df']["index"].unique()))) * 100
                similarity_dict[un_col] = res
            max_sim_col = max(similarity_dict, key=similarity_dict.get)
            logging.info(
                f'Column "{max_sim_col}" has similarity percentage with Target Index: {similarity_dict[max_sim_col]}%')
            # Rename it to index
    ...
    def __find_dataset_key(self):
        ...
        for name, df_dict in self.dfs.items():
            if df_dict['df'][df_dict['df'].duplicated(subset=['index', 'groups'], keep=False)].shape[0] == 0:
                logging.info(f'They key of the dataset {name} is the combination "Index, Group"')
    ...
    def __merged_df_eda(self):
        ...
        if self.generate_plots:
            ...
            for col in categ_col:
                self.df_merged[col] = self.df_merged[col].cat.remove_unused_categories()

            # Boxplots with target
            fig, ax = plt.subplots(4, math.ceil(len(categ_col) / 4), figsize=(len(categ_col) * 2, len(categ_col) * 3))
            for variable, subplot in zip(categ_col, ax.flatten()):
                sns.boxplot(x=variable, y=self.target, data=self.df_merged, ax=subplot)
            plt.savefig(f'plots/Merged_dataset_boxplots_with_target.png', bbox_inches='tight')
            logging.info('\nMerged dataset boxplots exported.')

            # Scatterplots with target
            numer_cols = self.df_merged.select_dtypes(exclude=['category', 'object', 'datetime']).columns
            fig, ax = plt.subplots(3, 3, figsize=(15, 10))
            for variable, subplot in zip(numer_cols, ax.flatten()):
                sns.scatterplot(x=variable, y=self.target, data=self.df_merged, ax=subplot)
            plt.savefig(f'plots/Merged_dataset_scatterplots_with_target.png', bbox_inches='tight')
            logging.info('Merged dataset scatterplots exported.')

            # Correlation plot
            fig, ax = plt.subplots(figsize=(17, 12))
            sns.heatmap(self.df_merged.corr(method='pearson'), annot=True, fmt='.4f',
                        cmap=plt.get_cmap('coolwarm'), cbar=False, ax=ax)
            plt.savefig(f'plots/Merged_dataset_correlations.png')
            logging.info('Correlation plot exported.')

            # Autocorrelation plot
            tsaplots.plot_acf(self.df_merged.sort_values(by=['when'])['target'], lags=40)
            ...
            for variable, subplot in zip(numer_cols, ax.flatten()):
                sns.lineplot(x='start_process', y=variable, data=merged_oneindex, ax=subplot)
            plt.savefig(f'plots/Merged_dataset_scatterplots_with_timestamp_index_1.png', bbox_inches='tight')

            tsaplots.plot_acf(merged_oneindex.sort_values(by=['when'])['target'], lags=40)
            plt.savefig(f'plots/Autocorrelation_in_target_index_1.png')
            logging.info('Merged dataset scatterplots with_timestamp exported index 1.')
    ...
    def __feature_engineering(self):
        ...
        # Try to use the timestamp columns by creating new ones that could be useful
        self.df_merged = timestamp_substractions(self.df_merged, 'process_duration', 'start_process', 'process_end')
        self.df_merged = timestamp_substractions(self.df_merged, 'subprocess1_duration', 'start_subprocess1',
                                                 'subprocess1_end')
        self.df_merged = timestamp_substractions(self.df_merged, 'time_to_start_subprocess1', 'start_process',
                                                 'start_subprocess1')
        self.df_merged = timestamp_substractions(self.df_merged, 'time_to_start_critsubprocess1', 'start_subprocess1',
    ...
    def __handle_nan_values(self):
        ...
        for col in self.df_merged.select_dtypes(include=['category', 'object']).columns:
            self.df_merged = self.df_merged[self.df_merged[col].notna()]
        # Nan to zeros for numerical variables
        for col in self.df_merged.select_dtypes(include=['int16', 'int32', 'float']).columns:
            self.df_merged[col] = self.df_merged[col].fillna(0)
        logging.info(
            f'{self.df_merged[self.df_merged.isna().any(axis=1)].shape[0]} observations with NaN values left.')
            ...
            for i in range(len(correlation_matrix.columns)):
                for j in range(i):
                    if abs(correlation_matrix.iloc[i, j]) >= cor:
                        logging.info(
                            f'Variables {correlation_matrix.columns[i]} and {correlation_matrix.columns[j]} are '
                            f'correlated with correlation coefficient {abs(correlation_matrix.iloc[i, j]).round(2)}')
                        colname = correlation_matrix.columns[i]
                        correlated_features.add(colname)
            # Drop them
            df = df.drop(list(correlated_features), axis=1)
            logging.info(f'Variables {list(correlated_features)} have been removed.')
            return df
    ...
    def __data_transformation(self):
        ...
        # Aggregation
        logging.info('\nAggregating observations with equal feature values and different target values...')
        logging.info(
            f'Examples of observations with same values in all features except from the target value:\n')
        example_agg = self.df_merged[self.df_merged.duplicated(
            subset=[i for i in self.df_merged.columns if i != self.target], keep=False)]\
            .sort_values("crystal_weight", ascending=False).head(6)
        logging.info(f'{example_agg}')
        self.df_merged = self.df_merged.groupby([i for i in self.df_merged.columns if i != self.target],
